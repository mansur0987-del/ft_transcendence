...
// ../node_modules/vue-use-kit/dist/vue-use-kit.es5.js
...
function useClickAway(elRef, callback, events) {
  ...
  onMounted(function() {
    events.forEach(function(evtName) {
      return document.addEventListener(evtName, handler);
    });
  });
...
function useGeolocation(options, runOnMount) {
  ...
  var geo = ref(__assign({}, geoInitData));
...
function useIdle(ms, events, runOnMount) {
  ...
  var start = function() {
    if (isTracking.value)
      return;
    events.forEach(function(evtName) {
      return document.addEventListener(evtName, handleChange);
    });
...
var fallbackToString = function(val) {
...
function useLocation(runOnMount) {
  ...
  var isTracking = ref(false);
  var locationState = ref(buildState("load"));
  var popState = function() {
    return locationState.value = buildState("popstate");
  };
  var pushState = function() {
    return locationState.value = buildState("pushstate");
  };
  var replaceState = function() {
    return locationState.value = buildState("replacestate");
  };
  var start = function() {
    if (isTracking.value)
      return;
...
function useMedia(query, defaultState2) {
  if (defaultState2 === void 0) {
    defaultState2 = false;
  }
  var mql;
  var matches = ref(defaultState2);
  var updateMatchValue = function() {
    return matches.value = mql.matches;
  };
  onMounted(function() {
...
function useMediaDevices(runOnMount) {
  ...
  var devicesState = ref([]);
  var isTracking = ref(false);
  var isTracked = ref(false);
...
function useOrientation(initialState, runOnMount) {
  ...
  var isTracking = ref(false);
  var orientation = ref(initialState);
...
var normalizeParams = function(urlParamsObj) {
  return function(paramAcc, param) {
...
function useSearchParams(parameters, runOnMount) {
  ...
  var searchParams = ref(parameters.reduce(normalizeParams({}), {}));
  var isTracking = ref(false);
...
function useScrollbarWidth(runOnMount) {
  ...
  onMounted(function() {
    return runOnMount && getScrollbarWidth();
  });
...
function useWindowSize(runOnMount) {
  ...
  var width = ref(isClient ? window.innerWidth : Infinity);
  var height = ref(isClient ? window.innerHeight : Infinity);
  var isTracking = ref(false);
...
var getFps = function(fps) {
  return Number(isRef(fps) ? fps.value : fps);
};
var calcFpsInterval = function(fps) {
  return 1e3 / fps;
};
var fpsLimit = 60;
function useRafFn(callback, fps, runOnMount) {
  if (fps === void 0) {
    fps = fpsLimit;
  }
  if (runOnMount === void 0) {
  ...
  var isRunning = ref(false);
  var fpsInterval = computed(function() {
    return calcFpsInterval(getFps(fps));
  });
  var itWasIdle = false;
  var startTime = 0;
  var timeElapsed = 0;
...
var Cookies = function() {
  var state = {
    ...
    remove: function(name, opts) {
      ...
      if (opts === void 0) {
        opts = { path: "/", expires: /* @__PURE__ */ new Date(0) };
      }
      var cookie$$1 = state.get(name);
      opts.expires = /* @__PURE__ */ new Date(0);
...
function useCookie(cookieName, options, runOnMount) {
  ...
  var cookie2 = ref(null);
  var getCookie = function() {
    var cookieVal = tryDeserialize(cookieLib.get(cookieName), deserializer, isParsing);
    if (!isNullOrUndefined(cookieVal))
      cookie2.value = cookieVal;
  };
  var setCookie = function(newVal, options2) {
    var newCookieVal = trySerialize(newVal, serializer, isParsing);
    cookieLib.set(cookieName, newCookieVal, options2);
    cookie2.value = tryDeserialize(newCookieVal, deserializer, isParsing);
  };
  var removeCookie = function(options2) {
    cookieLib.remove(cookieName, options2);
    cookie2.value = null;
  };
  onMounted(function() {
    return runOnMount && getCookie();
  });
  return { cookie: cookie2, getCookie, setCookie, removeCookie };
}
...
var fetchWrapper = function(url, opts) {
  return __awaiter(void 0, void 0, void 0, function() {
    var res, resData, _a;
    return __generator(this, function(_b) {
      switch (_b.label) {
        case 0:
          return [4, fetch(getUrl(url), opts)];
        case 1:
          res = _b.sent();
      ...
      return __generator(this, function(_b) {
        switch (_b.label) {
          case 0:
            ...
            return [4, fetchWrapper(getUrl(url), __assign({ signal }, options))];
          case 1:
...
vue-use-kit/dist/vue-use-kit.es5.js:
  (*! *****************************************************************************
  ...
  ***************************************************************************** *)
   ...
   *)
*/
//# sourceMappingURL=vue-use-kit.js.map
