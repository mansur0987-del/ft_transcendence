...
export class PlayerService {
  ...
  async create(playerDto: CreatePlayerDto): Promise<PlayerEntity> {
    const file: Buffer = await fs.readFileSync(path.resolve('../user.png'));
  ...
  async addAvatar(userId: number, imageBuffer: Buffer, filename: string) {
    const avatar = await this.avatarService.uploadDatabaseFile(
      imageBuffer,
      filename,
    );
  ...
  async addQrCode(userId: number, imageBuffer: Buffer, filename: string) {
    const QrCode = await this.qrCodeServise.uploadDatabaseFile(
      imageBuffer,
      filename,
    );
    await this.player_repository.update(userId, {
  ...
  async getSendPlayerApplication(sendPlayerId: number) {
    ...
    const playerSendApplication =
      await this.playerApplication.getSendPlayerApplication(sendPlayerId);
    const players: PlayerEntity[] = [];
    for (let i = 0; playerSendApplication[i]; i++) {
      const tmp_player = await this.GetPlayerById(
        playerSendApplication[i].getPlayerId,
      );
  ...
  async getGetPlayerApplication(getPlayerId: number) {
    ...
    const playerGetApplication =
      await this.playerApplication.getGetPlayerApplication(getPlayerId);
    const players: PlayerEntity[] = [];
    for (let i = 0; playerGetApplication[i]; i++) {
      const tmp_player = await this.GetPlayerById(
        playerGetApplication[i].sendPlayerId,
      );
      players.push(tmp_player);
    }
    return { player, players };
  ...
  async setApplication(
    ...
    if (sendPlayerId == getPlayerId) {
      throw new HttpException(
        'player who send application and player who get application are the same',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (!(await this.GetPlayerById(sendPlayerId))) {
      throw new HttpException(
        'player who send application not found',
        HttpStatus.BAD_REQUEST,
      );
    }
    if (!(await this.GetPlayerById(getPlayerId))) {
      throw new HttpException(
        'player who get application not found',
        HttpStatus.BAD_REQUEST,
      );
    }
    const application =
      await this.playerApplication.getGetAndSendPlayerApplication(
        getPlayerId,
        sendPlayerId,
      );
    if (application) {
      ...
      await this.playerApplication.setSendPlayerApplication(
        sendPlayerId,
        getPlayerId,
      );
    }
  ...
  async removeApplication(
    ...
    await this.playerApplication.deleteGetAndSendPlayerApplication(
      sendPlayerId,
      getPlayerId,
    );
    return true;
