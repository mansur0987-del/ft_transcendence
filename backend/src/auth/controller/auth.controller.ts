...
export class AuthController {
	...
	//@Get('signin')
	//public async signin(@Body() data: UsernamePasswordDTO, @Request() req) {
	//	return await this.authService.signIn(data, req);
	//}
	...
	async turn_off(@Request() req, @Body() body): Promise<ReturnStatus>{
		return await this.authService.turn_off(req.user, body.twoFactorAuthenticationCode)
	}

	@Post('2fa/generate')
	@UseGuards(AuthGuard('jwt'))
	async register(@Request() req, @Response() res): Promise<ReturnStatus> {
		return await this.authService.register(req.user, res)
	}
	...
	async authenticate(@Request() req, @Body() body): Promise<ReturnStatus> {
		return await this.authService.authenticate(req.user, body.twoFactorAuthenticationCode)
	}
}
